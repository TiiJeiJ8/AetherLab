...
def parse_contents(contents, filename):
    ...
    try:
        if 'csv' in filename:
            ...
            return None, f"不支持的文件类型: {filename}"
            
        # Basic data cleaning
        df = df.dropna(how='all')  # Drop empty rows
        df = df.loc[:, ~df.columns.str.contains('^Unnamed')]  # Drop unnamed columns
        
        return df, f"成功加载文件: {filename}"
...
data_preprocessing_layout = html.Div([
    ...
    html.Div(children=[
        ...
        dbc.Container([
            ...
            dbc.Row([
                ...
                dbc.Col([
                    dbc.Button(
                        '数据可视化', id='data-visualization-button', className="btn btn-primary btn-animated", href='/chart-visualizations', disabled=False,
                        style={
                    ...
                    dbc.DropdownMenu(
                        [
                            dbc.DropdownMenuItem("数据分析", id="data-preprocess-btn", href='/data-preprocessing', disabled=True),
                            dbc.DropdownMenuItem("数据聚类 (待开发)", id="data-cluster-btn", disabled=True),
                            dbc.DropdownMenuItem("数学建模分析 (待开发)", id="data-export-btn", disabled=True),
                        ],
                        label="数据处理与分析",
                    ...
                    dbc.Button(
                        'About ME', href="https://tiijeij8.github.io/", id='about-button', className="btn btn-primary", disabled=False,
                        style={
                            'margin-top': '1.1rem', 'margin-right': '1rem',
    ...
    dbc.Container([
        ...
        dbc.Row([
            ...
            dbc.Col([
                dbc.Tabs([
                    dbc.Tab(label="数据可视化", tab_id="visualization"),
                    dbc.Tab(label="分析报告", tab_id="analysis"),
...
def render_analysis_report(report):
    ...
    overview_card = dbc.Card([
        ...
        dbc.CardBody([
            dbc.Row([
                dbc.Col([
                    html.Div([
                        html.H5("数据集概况", className="card-title mb-4 text-primary"),
                        html.Div([
                            html.P("总行数:", className="font-weight-bold"),
                            html.P(f"{report['overview']['rows']}", className="text-muted"),
                            html.P("总列数:", className="font-weight-bold mt-2"),
                            html.P(f"{report['overview']['columns']}", className="text-muted"),
                            html.P("数值列数量:", className="font-weight-bold mt-2"),
                            html.P(f"{len(report['overview']['numeric_cols'])}", className="text-muted"),
                            html.P("分类列数量:", className="font-weight-bold mt-2"),
                            html.P(f"{len(report['overview']['categorical_cols'])}", className="text-muted"),
                            html.P("日期列数量:", className="font-weight-bold mt-2"),
                ...
                dbc.Col([
                    html.H5("各列类型说明", className="card-title mb-4 text-primary"),
                    html.Div([
                        html.P("数值型列（支持数学运算）:", className="font-weight-bold mb-1"),
                        html.P(", ".join(report['overview']['numeric_cols'] or ['无']), className="mb-3"),
                        html.P("分类型列（文本或类别数据）:", className="font-weight-bold mb-1"),
                        html.P(", ".join(report['overview']['categorical_cols'] or ['无']), className="mb-3"),
                        html.P("日期型列（时间数据）:", className="font-weight-bold mb-1"),
                        html.P(", ".join(report['overview']['date_cols'] or ['无']), className="mb-3")
    ...
    missing_card = dbc.Card([
        dbc.CardHeader("缺失值统计", className="bg-info text-white"),
        dbc.CardBody([
            dcc.Graph(figure=missing_fig),
            html.Div([
                html.P("缺失值最多的列:", className="font-weight-bold"),
                html.P(max(report['missing_values'].items(), key=lambda x: x[1])[0] if report['missing_values'] else "无")
            ], className="mt-3")
        ])
    ], className="mb-4")
    ...
    if report['descriptive_stats']:
            ...
            stats_content.append(dbc.Card([
                ...
                dbc.CardBody([
                    dbc.Row([
                        dbc.Col([
                            html.Div([
                                html.P(f"非空值数: {stats.get('count', 'N/A')}"),
                                html.P(f"平均值: {stats.get('mean', 'N/A'):.2f}"),
                                html.P(f"标准差: {stats.get('std', 'N/A'):.2f}"),
                                html.P(f"最小值: {stats.get('min', 'N/A')}"),
                                html.P(f"25分位数: {stats.get('25%', 'N/A')}"),
                                html.P(f"中位数: {stats.get('50%', 'N/A')}"),
                                html.P(f"75分位数: {stats.get('75%', 'N/A')}"),
                                html.P(f"最大值: {stats.get('max', 'N/A')}")
                            ])
                        ], width=4),
                        dbc.Col([
        ...
        stats_content.append(html.P("没有数值列数据"))

    stats_card = dbc.Card([
        dbc.CardHeader("描述性统计 (数值列)", className="bg-warning text-dark"),
